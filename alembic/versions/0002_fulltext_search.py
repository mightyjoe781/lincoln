...
def upgrade():
    ...
    op.create_index('ix_invoices_search_vector', 'invoices', ['search_vector'], postgresql_using='gin')
    op.execute("""
    ...
    op.create_index('ix_transactions_search_vector', 'transactions', ['search_vector'], postgresql_using='gin')
    op.execute("""
    ...
    """)
    op.execute("UPDATE transactions SET search_vector = to_tsvector('english', COALESCE(description, ''))")
